#!/usr/bin/env perl
#
#  FASTA Splitter  -  a script for dividing FASTA files into parts
#
#  Version 0.2.7 (September 13, 2024)
#
#  Copyright (c) 2012-2024 Kirill Kryukov
#
...
my @files = ();
my ($opt_n_parts,$opt_part_size,$opt_part_num_prefix,$opt_measure,$opt_line_len,$opt_eol,$out_dir,$nopad,$version,$help);
...
if ($version) { print "fasta-splitter 0.2.7\n"; exit(0); }

if ($help)
{
    print qq~Usage: fasta-splitter.pl [Options] <File>...
Options:
...
if (!defined($opt_n_parts) and !defined($opt_part_size)) { print "Splitting method is not specified\nUse -h for help\n"; exit(0); }
...
my $line_len = (defined($opt_line_len) and $opt_line_len >= 0) ? $opt_line_len : 100;
